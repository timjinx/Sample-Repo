...
{
     printf("Usage: %s [--dos2unix,--unix2dos,-a] -i <input file> -o <output file>\n", p_name);
}
...
int main(int argc, char** argv)
{
     ...
     if ( ( dos2unix + unix2dos + v_auto ) != 1 )  {
         puts("Select either dos2unix or unix2dos, or auto if you are feeling brave");
         helptext(argv[0]);
         exit(EXIT_FAILURE);
     }
     if ( v_auto == 1 ) {
     ...
     while ((read = getline(&line, &len, ifptr)) != -1) {
         if ( v_auto == 1) {
             ...
             strip(line);
             if ( (strlen(line) + 1) < strlen(testline) ) {
                 dos2unix=1;
             } else {
                 unix2dos=1;
             }
